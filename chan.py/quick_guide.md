...
## 写在前面
- 项目初衷：本项目致力于实现一套完整的缠论自动化分析与交易框架。通过程序化描述股票走势，实现对市场机会的精准捕捉。
- 核心功能：
  - 缠论全元素（笔、线段、中枢、买卖点）的自动化计算。
  - 支持多级别联立分析与区间套策略。
  - 开放的架构设计，支持自定义数据接入、策略开发及机器学习模型集成。
- 开发目标：基于现有的开源框架，进一步完善并生成完整的技术文档与交易策略，构建一个高效、稳定的量化交易基座。
- 欢迎交流：欢迎对缠论量化感兴趣的朋友提交 PR 或通过邮件共同完善本项目。
- 视频参考：[20230909交流会录屏](https://www.bilibili.com/video/BV1nu411c7oG/);
...
### 关于完整文档
本项目的目标是基于现有开源基座，逐步完善所有模块的详细技术文档，并根据开发进度更新。

开源的部分提供了所有可以用到的基座和演示代码，已经足够大家自行用于组装自己的策略。
...
def DrawElement(self, ...):
    ...
    if plot_config.get("plot_rsi", False):
        self.draw_rsi(meta, kline, **plot_para.get('rsi', {}))
```

然后实现draw_rsi方法即可：
```python
def draw_rsi(self, meta: CChanPlotMeta, x_limits, color='b'):
    data = [klu.rsi for klu in list(meta.klu_iter())[x_limits[0]:x_limits[1]+1]]
    return Line().add_xaxis(xaxis_data=meta.datetick[x_limits[0]:x_limits[1]+1]).add_yaxis(
        series_name="RSI",
        y_axis=data,
